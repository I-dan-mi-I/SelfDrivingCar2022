...
# подключение самописных API, чтобы облегчить жизнь Даньке
from Drawer import Drawer
from ShapeObjects import *
from PygameAdditionalMethods import *

# чтобы ворочать числа
import numpy as np

# инициализация объектов Drawer и Vector2
drawer = Drawer()
...
class Game:
    ...
    def __init__(self):
        # даже если помяняете картинку, стены всё равно будут на месте
    ...
    def render(self):
        # этот блок можно менять так, чтобы изменить отображение различных объктов

        glPushMatrix()

        # вы можете отключить это, но тогда картинка станет адом (в прямом смысле)
        self.trackSprite.draw()

        # угадай, где мамин гонщик
        self.car.show()
...
class RewardGate:
    """Чекпоинт. вариант, который больше мне нравится: ВРАТА НАГРАДЫ"""
...
class Car:
    ...
    def show(self):
        ...
        # если отключить это, то отключится отображение пересечений прямых
